...
class TwitchService(twitchio.Client):
    ...
    def __init__(self):
        ...
        self._redis_client = redis.Redis.from_url(settings.REDIS_URL or 'redis://redis:6379/0')
    ...
    async def _create_event_from_payload(self, event_type: str, payload):
        ...
        try:
            ...
            logger.info(f"Created event: {event_type} from {member.display_name if member else 'unknown'}")
    ...
    async def _publish_to_redis(self, event_type: str, event: Event, member: Optional[Member], payload_dict: dict):
        ...
        try:
            ...
            channel = f'events:twitch'
            message_json = json.dumps(redis_message)
            
            # Use asyncio to run Redis publish in thread pool
            loop = asyncio.get_event_loop()
            await loop.run_in_executor(None, self._redis_client.publish, channel, message_json)
    ...
    async def _get_or_create_member_from_payload(self, payload) -> Optional[Member]:
        """Extract Member information from EventSub payload."""
        # Try different user ID fields depending on event type
        twitch_id = (
            getattr(payload, 'user_id', None) or
            getattr(payload, 'from_broadcaster_user_id', None) or
            getattr(payload, 'broadcaster_user_id', None)
        )
        
        username = (
            getattr(payload, 'user_login', None) or
            getattr(payload, 'from_broadcaster_user_login', None) or
            getattr(payload, 'broadcaster_user_login', None)
        )
        
        display_name = (
            getattr(payload, 'user_name', None) or
            getattr(payload, 'from_broadcaster_user_name', None) or
            getattr(payload, 'broadcaster_user_name', None)
        )
    ...
    async def _create_event(self, event_type: str, payload: dict, member: Optional[Member], source_id: Optional[str] = None) -> Event:
