...
class TwitchService(twitchio.Client):
    ...
    def __init__(self):
        ...
        # Type-specific event handler mapping - CRITICAL for proper payload handling
        # Import and initialize after TwitchIO is available
        ...
        try:
            ...
            for token in tokens:
                if not token.is_expired:
                    ...
                    logger.info(
                        f"Token for user {token.user_id} has expired, attempting refresh"
                    )
                    try:
                        ...
                        logger.error(
                            f"Error refreshing token for user {token.user_id}: {e}"
                        )
    ...
    async def _handle_chat_message(self, event_type: str, payload):
        ...
        payload_dict = {
            ...
            "color": payload.color,
            "badges": [{"set_id": badge.set_id, "id": badge.id} for badge in payload.badges] if payload.badges else [],
            "emotes": [
                {"id": e.id, "name": e.name, "positions": e.positions}
                for e in payload.emotes
            ]
            if payload.emotes
            else [],
    ...
    async def _handle_channel_cheer(self, event_type: str, payload):
        ...
        payload_dict = {
            ...
            "anonymous": getattr(payload, "anonymous", False),
            "broadcaster_user_id": payload.broadcaster.id,
            "broadcaster_user_name": payload.broadcaster.name,
            "broadcaster_user_login": payload.broadcaster.login,
            "type": payload.type if hasattr(payload, "type") else None,
            "fragments": [
                {"text": frag.text, "type": getattr(frag, "type", None)}
                for frag in payload.fragments
            ]
            if hasattr(payload, "fragments") and payload.fragments
            else [],
        }
        member = await self._get_or_create_member_from_payload(payload)
        event = await self._create_event(event_type, payload_dict, member)
    ...
    async def _handle_channel_raid(self, event_type: str, payload):
        ...
        payload_dict = {
            ...
            "from_broadcaster_user_login": payload.from_broadcaster.login,
    ...
    async def _handle_chat_notification(self, event_type: str, payload):
        ...
        payload_dict = {
            ...
            "chatter_user_login": payload.chatter.login if payload.chatter else None,
    ...
    async def _handle_chat_message_delete(self, event_type: str, payload):
        ...
        payload_dict = {
            ...
            "target_user_login": payload.target_user.login,
    ...
    async def _handle_channel_ban(self, event_type: str, payload):
        ...
        payload_dict = {
            ...
            "moderator_user_login": payload.moderator.login
            if payload.moderator
            else None,
    ...
    async def _handle_ad_break_begin(self, event_type: str, payload):
        ...
        payload_dict = {
            ...
            "requester_user_login": payload.requester.login
            if hasattr(payload, "requester") and payload.requester
            else None,
        }
        member = await self._get_or_create_member_from_payload(payload)
